...
## Observações
- A entidade foi nomeada `Adress` conforme o código presente no projeto (atenção à grafia). Se preferir renomear para `Address`, eu posso ajustar todo o projeto.
- O projeto usa `spring.jpa.hibernate.ddl-auto=update` para criar/atualizar o esquema automaticamente em desenvolvimento. Para produção, considere usar Flyway ou Liquibase.

## Próximos passos sugeridos
- Adicionar testes unitários para o `AddressServiceImpl` (mock do RestTemplate)
- Persistir logs de consulta usando `AddressLogRepository` e gravar no service
- Tornar `baseUrl` da API externa configurável via `application.properties`

---

Se quiser, eu posso:
- Gerar o export do Mockoon para você importar rapidamente;
- Criar um exemplo de `AddressLogRepository` e integrar a gravação de logs no `AddressServiceImpl`.

Digite o que prefere e eu implemento.
